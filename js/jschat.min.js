...
this.connect();this.chatlog.bind("add",this.callback("onMessageAdd"));this.view.bind("send:message",this.callback("sendMessage"))},connect:function(){this.connection.connect(this.options.jid,this.options.password,this.callback("onConnectChange"));this.trigger("ui:connect")},onConnectChange:function(a){for(st in Strophe.Status);a===Strophe.Status.CONNECTED&&this.trigger("connected")},onConnect:function(){this.connection.sendIQ($iq({type:"get"}).c("query",{xmlns:"jabber:iq:roster"}),this.callback("onRoster"));
this.trigger("ui:roster");this.connection.addHandler(this.callback("onContactPresence"),null,"presence");this.connection.addHandler(this.callback("onMessage"),null,"message","chat")},onRoster:function(a){this.connection.send($pres());this.trigger("ui:ready");this.view.setStatus(Jschat.viewstates.online);for(var a=Jschat.Roster.serializeRoster(a),c=0;c<a.length;c++)this.roster.add(a[c]);return!0},onContactPresence:function(a){var c=Strophe.getBareJidFromJid($(a).attr("from")),b=this.roster.detect(function(a){return a.get("bare_jid")===
c});b&&b.updatePrecense(a);this.options.autoChat&&_.delay(function(a){a.sendWelcome()},"2000",this);return!0},sendWelcome:function(){if(!this._welcomeSent){var a=this.getUserinfo();this.roster.freezeManager();this._welcomeSent=!0;this.sendMessage({text:a,from:this.options.jid,to:this.roster.manager.get("jid"),hidden:!0,dt:new Date})}},sendMessage:function(a){this._welcomeSent||this.sendWelcome();a=typeof a==="string"?new Jschat.Message({text:a,from:this.options.jid,to:this.roster.manager.get("jid"),
incoming:!1,dt:new Date}):new Jschat.Message(a);a.send(this.connection);a.get("hidden")||this.chatlog.add(a)},getUserinfo:function(){return Jschat.welcome_template(this.view.getUserinfo())},onMessage:function(a){this.chatlog.add(new Jschat.Message({text:$(a).find("body").text(),from:$(a).attr("from"),to:$(a).attr("to"),incoming:!0,dt:new Date}));return!0},onMessageAdd:function(a){this.view.trigger("add:message",a)}});